...
# Versión v9 - Relaciones automáticas del Módulo I y vigencia reactiva
# ============================================================
# Base funcional:
...
# - Contextos automáticos persona-hogar-predio-vivienda-activo.
...
def mostrar_contextos_automaticos(
    ...
    """
    Muestra relaciones reconocidas automáticamente.

    Todos los campos son de solo lectura. Cuando no existe información,
    se muestran bloqueados en gris con el texto correspondiente.
    """
    relaciones, contexto = relaciones_automaticas_entidad(nivel, id_entidad)

    st.markdown("#### Contexto reconocido automáticamente")
    st.caption(
        "Información consultada desde los datos maestros del Módulo I. "
        "Estos campos no se capturan nuevamente en Gestión Documental."
    )
    ...
    for inicio in range(0, len(orden_contextos), 3):
        columnas = st.columns(3)
        for posicion, tipo in enumerate(orden_contextos[inicio:inicio + 3]):
            valor = _referencia_contexto(tipo, contexto.get(tipo, []))
            columnas[posicion].text_input(
                tipo,
                value=valor,
                disabled=True,
                key=f"{key_prefix}_auto_{tipo}",
            )
...
def selector_relaciones_adicionales(
    ...
    id_principal: str,
    key_prefix: str,
    relaciones_automaticas: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    """
    Permite agregar únicamente relaciones excepcionales que no pudieron
    inferirse desde los datos maestros.
    """
    relaciones = list(relaciones_automaticas)
    existentes = {
        (str(item.get("tipo_entidad")), str(item.get("id_entidad")))
        for item in relaciones
    }

    with st.expander("Relaciones adicionales (opcionales)", expanded=False):
        st.caption(
            "Utilice esta sección solo cuando la relación no exista en los "
            "datos maestros. El archivo seguirá almacenándose una sola vez."
        )
