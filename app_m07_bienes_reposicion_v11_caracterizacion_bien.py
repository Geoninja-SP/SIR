...
def aplicar_estilos():
    ...
    st.markdown(
        ...
        <style>
            ...
            .question-card {{ border:1px solid var(--sir-border); border-radius:18px; padding:.85rem .95rem; background: color-mix(in srgb, var(--sir-card) 90%, var(--sir-accent) 5%); margin-bottom:.6rem; }}
            .question-kicker {{ color: color-mix(in srgb, var(--sir-text) 72%, var(--sir-accent) 28%); font-size:.72rem; font-weight:900; text-transform:uppercase; letter-spacing:.07em; margin-bottom:.28rem; }}
            .question-text {{ color:var(--sir-text); font-weight:850; line-height:1.3; margin-bottom:.35rem; }}
            .question-meta {{ color: color-mix(in srgb, var(--sir-text) 68%, var(--sir-primary) 32%); font-size:.8rem; }}
...
def render_pregunta_catalogo(pregunta, valor_actual, key_prefix):
    """Renderiza una pregunta de caracterización con catálogo simple o múltiple."""
    campo = pregunta["campo"]
    opciones = pregunta.get("opciones", [])
    st.markdown(
        f"""
        <div class='question-card'>
            <div class='question-kicker'>{escape(pregunta['seccion'])} · {escape(pregunta['capital'])}</div>
            <div class='question-text'>{escape(pregunta['pregunta'])}</div>
            <div class='question-meta'>{escape(pregunta['dimension'])}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
    if pregunta.get("tipo") == "Catálogo múltiple":
        ...
        seleccion = st.multiselect(etiqueta(campo), opciones, default=default, key=f"{key_prefix}_{campo}")
        return "; ".join(seleccion)
    index = indice_seguro(opciones, valor_actual)
    return st.selectbox(etiqueta(campo), opciones, index=index, key=f"{key_prefix}_{campo}")
...
def pantalla_caracterizacion_bien_repuesto(filtros_globales=None):
    st.markdown("### Caracterización del bien repuesto")
    st.markdown(
        '<div class="screen-help">Captura la caracterización socioeconómica y de condiciones habitacionales del bien de reposición familiar, usando preguntas generalizadas por catálogo vinculadas a IFC PS5.</div>',
        unsafe_allow_html=True,
    )
    bienes = obtener_df("bienes_reposicion")
    ...
    filtrado = filtrar_caracterizacion(caracterizaciones, filtros_globales)
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Bienes familiares", len(bienes))
    c2.metric("Caracterizaciones", len(caracterizaciones))
    c3.metric("Pendientes", max(len(bienes) - len(caracterizaciones), 0))
    pct = round((len(caracterizaciones) / len(bienes)) * 100, 1) if len(bienes) else 0
    c4.metric("Avance", f"{pct}%")

    st.markdown("#### Registros de caracterización")
    mostrar_dataframe(
        filtrado,
        [
            "id_caracterizacion", "fecha_registro", "id_hogar", "id_bien_reposicion", "numero_registro_componente",
            "tipo_bien_reposicion", "clase_vivienda", "material_piso", "situacion_energia_electrica",
            "disponibilidad_agua_entubada", "tipo_servicio_sanitario", "tipo_conexion_drenaje",
            "infraestructura_almacenamiento_agua",
        ],
    )

    if not filtrado.empty:
        st.markdown("#### Checklist de servicios básicos del registro visible")
        id_detalle = st.selectbox(
            "Selecciona caracterización para ver checklist",
            filtrado["id_caracterizacion"].astype(str).tolist(),
            key="detalle_check_caracterizacion",
        )
        registro_check = filtrado[filtrado["id_caracterizacion"].astype(str) == str(id_detalle)].iloc[0].to_dict()
        mostrar_check_servicios_basicos(registro_check)

    st.markdown("---")
    st.markdown("#### Registrar o actualizar caracterización")
    ...
    id_bien = colb.selectbox(
        ...
        ids_bien,
        format_func=lambda x: f"{x} · {bienes_hogar[bienes_hogar['id_bien_reposicion'].astype(str)==str(x)].iloc[0].get('numero_registro_componente', '')} · {bienes_hogar[bienes_hogar['id_bien_reposicion'].astype(str)==str(x)].iloc[0].get('tipo_bien_reposicion', '')}",
    ...
    st.markdown(
        f"{chip('Hogar: ' + str(id_hogar))} {chip('Bien: ' + str(id_bien), 'success')} {chip('Registro: ' + str(bien.get('numero_registro_componente', '')))}",
        unsafe_allow_html=True,
    )
    ficha_visual(
        "Contexto del bien repuesto",
        bien,
        ["id_bien_reposicion", "numero_registro_componente", "id_paquete_compensacion", "tipo_bien_reposicion", "descripcion_bien", "ubicacion_bien", "estado_proceso", "valor_referencial_usd"],
    )

    with st.form(f"form_caracterizacion_{id_bien}"):
        col1, col2, col3 = st.columns(3)
        id_caracterizacion = col1.text_input("ID caracterización", value=str(base.get("id_caracterizacion", "")), disabled=True)
        fecha_registro = col2.date_input("Fecha de registro", value=base.get("fecha_registro") if isinstance(base.get("fecha_registro"), date) else date.today())
        col3.text_input("Número único de componente", value=str(bien.get("numero_registro_componente", "")), disabled=True)

        respuestas = {}
        grupos = []
        for pregunta in PREGUNTAS_CARACTERIZACION:
            grupo = pregunta["seccion"].split(" - ")[0]
            if grupo not in grupos:
                grupos.append(grupo)
        for grupo in grupos:
            with st.expander(grupo, expanded=grupo in ["Clase de vivienda particular", "Características de la vivienda"]):
                for pregunta in [p for p in PREGUNTAS_CARACTERIZACION if p["seccion"].split(" - ")[0] == grupo]:
