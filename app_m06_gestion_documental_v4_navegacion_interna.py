...
def crear_o_actualizar_expediente(
    ...
    # Regla de unicidad: solo puede existir un expediente por lugar poblado,
    # un expediente por hogar y un expediente por persona. Si ya existe,
    # se actualiza el mismo ID; nunca se crea un segundo expediente.
...
def guardar_documento(registro):
    # Regla productiva: el cargador y el revisor deben ser personas diferentes.
    # Excepción temporal: MODO_BETA_AUTORREVISION permite probar el flujo completo
    # con el mismo usuario sin cambiar de sesión.
    if (
...
def formulario_documento(nivel, expediente):
    ...
    with st.form(f"form_documento_{nivel}_{id_exp}_{codigo_carpeta}"):
        ...
        if MODO_BETA_AUTORREVISION:
            revisor = st.session_state.usuario_actual
            st.text_input(
                "Responsable de revisión (beta)",
                value=revisor,
                disabled=True,
                help=(
                    "En producción este campo deberá asignarse a una persona distinta "
                    "de quien cargó el documento. En beta se permite autorrevisión."
                ),
            )
        else:
...
def mostrar_contexto_activo(nivel, expediente):
    """Muestra siempre el contexto activo para evitar capturas en el expediente equivocado."""
    st.markdown("#### Contexto activo")
    id_lugar = expediente.get("id_lugar_poblado", "")
    id_hogar = expediente.get("id_hogar", "")
    id_persona = expediente.get("id_persona", "")
    lugar = obtener_lugar(id_lugar) if id_lugar else {}
    hogar = obtener_hogar(id_hogar) if id_hogar else {}
    persona = obtener_persona(id_persona) if id_persona else {}

    cols = st.columns(4)
    cols[0].info(f"**Expediente:**\n\n{expediente.get('id_expediente', '')}")
    if nivel == "Lugares poblados":
        cols[1].info(f"**Lugar poblado:**\n\n{lugar.get('nombre_lugar_poblado', '')}")
        cols[2].info(f"**ID lugar:**\n\n{id_lugar}")
        cols[3].info(f"**Zona:**\n\n{lugar.get('zona', '')}")
    elif nivel == "Hogares":
        cols[1].info(f"**Hogar:**\n\n{hogar.get('nombre_referencia_hogar', '')}")
        cols[2].info(f"**ID hogar:**\n\n{id_hogar}")
        cols[3].info(f"**Lugar poblado:**\n\n{lugar.get('nombre_lugar_poblado', '')}")
    else:
        cols[1].info(f"**Hogar:**\n\n{hogar.get('nombre_referencia_hogar', '')}")
        cols[2].info(f"**Persona:**\n\n{persona.get('nombres', '')} {persona.get('apellidos', '')}")
        cols[3].info(f"**ID persona:**\n\n{id_persona}")
...
def tabla_documentos(expediente):
    ...
    cols = [
        "id_documento", "codigo_carpeta", "carpeta", "tipo_documental",
        "nombre_archivo", "ruta_archivo", "fecha_documento", "fecha_carga",
        "estado_carga", "estado_revision", "confirmado", "version",
    ]
    st.dataframe(
        sub[cols],
        use_container_width=True,
    ...
    st.markdown("#### Detalle y acceso al contenido")
    opciones = sub["id_documento"].astype(str).tolist()
    etiquetas = {
        r["id_documento"]: f"{r['id_documento']} · {r['tipo_documental']}"
    ...
    id_doc = st.selectbox(
        ...
        opciones,
        format_func=lambda x: etiquetas.get(x, x),
        key=f"detalle_doc_{expediente['id_expediente']}",
    )
    doc = sub[sub["id_documento"].astype(str).eq(id_doc)].iloc[0].to_dict()
    c1, c2, c3 = st.columns(3)
    c1.info(f"**Carpeta:**\n\n{doc.get('carpeta', '')}")
    c2.info(f"**Estado de revisión:**\n\n{doc.get('estado_revision', '')}")
    c3.info(f"**Versión:**\n\n{doc.get('version', '')}")
    st.write(f"**Archivo:** {doc.get('nombre_archivo') or 'Sin referencia'}")
    ...
    if enlace_valido(ruta):
        ...
        st.code(ruta)
        st.caption("La ruta se muestra completa. Para abrirla como enlace debe ser una URL http/https.")
    else:
        st.warning("Este documento no tiene vínculo o ruta registrada.")
    st.write(f"**Observaciones de carga:** {doc.get('observaciones_carga') or 'Sin observaciones'}")


def vista_historico(expediente):
    """Muestra historial de revisiones y cambios documentales del expediente."""
    docs = st.session_state.data_m06["documentos"]
    ...
    if sub_docs.empty:
        ...
        return
    movimientos = sub_docs[[
        "id_documento", "tipo_documental", "version", "fecha_carga",
        "usuario_carga", "estado_revision", "fecha_actualizacion", "ruta_archivo",
    ]].copy()
    st.dataframe(
        movimientos.sort_values("fecha_actualizacion", ascending=False),
        use_container_width=True,
        hide_index=True,
        column_config={
            "ruta_archivo": st.column_config.LinkColumn("Documento", display_text="Abrir"),
        },
    )
...
def formulario_expediente_y_tabs(nivel):
    ...
    if nivel == "Hogares":
        opciones.append("Checklist y progreso")
    vista = st.radio(
        "Vista de trabajo",
        opciones,
        horizontal=True,
        key=f"vista_interna_{nivel}",
    )

    if expediente:
        mostrar_contexto_activo(nivel, expediente)
    else:
        st.info("No existe expediente para el registro seleccionado. Créalo desde la vista Expediente.")
    ...
    if vista == "Resumen":
        ...
        c3.metric("Documentos", len(sub))
        c4.metric("Aprobados", int((sub["estado_revision"] == "Aprobado").sum()) if not sub.empty else 0)
        if nivel == "Hogares":
    ...
    elif vista == "Checklist y progreso" and nivel == "Hogares":
