...
def recalcular_progreso_expediente(id_expediente):
    """Calcula el progreso contra todo el catálogo del nivel del expediente."""
...
def sincronizar_checklist_documento(id_documento):
    """Sincroniza el requisito usando todas las cargas del mismo tipo documental."""
    ...
    expediente = expediente_existente(
        doc.get("nivel", ""),
        id_lugar=doc.get("id_lugar_poblado", ""),
        id_hogar=doc.get("id_hogar", ""),
        id_persona=doc.get("id_persona", ""),
    )
    if not expediente:
        return
    ...
    grupo = grupo.sort_values(["_orden", "_version"], ascending=[False, False])
...
def formulario_documento(nivel, expediente):
    ...
    form_key = f"form_documento_{nivel}_{id_exp}_{codigo_carpeta}_{codigo_documento}_{token}"
    ...
    if guardar:
        submission_id = f"{id_exp}|{codigo_documento}|{token}"
        procesados = st.session_state.setdefault("cargas_procesadas_m06", set())
        if submission_id in procesados:
            st.warning("Esta acción de guardado ya fue procesada.")
            return
        ...
        try:
            procesados.add(submission_id)
            guardar_documento(registro)
            st.session_state[token_key] = token + 1
            st.success(f"Documento registrado una sola vez como versión {version}.")
            st.rerun()
        except ValueError as e:
            procesados.discard(submission_id)
            st.error(str(e))
